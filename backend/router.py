...
def get_current_user(token: str = Depends(oauth2_scheme)):
    ...
    try:
        ...
        # Extract `sub` (username) and `user_type`
        username = payload.get("sub")
        user_type = payload.get("user_type")

        # Validate required fields in the token
        if username is None or user_type is None:
            print("Token does not contain required fields ('sub', 'user_type')")
        ...
        # Ensure both user_type and privilege fields exist
        user["_id"] = str(user["_id"])  # Convert `_id` to string
        user["user_type"] = user_type  # Set user_type from token
        if "privilege" not in user:
            user["privilege"] = user_type  # Use user_type as privilege if not present
        
        print("User Retrieved from DB:", user)  # Debugging user

        return user
    except JWTError as e:
        print(f"JWT Error: {e}")  # Debugging token-related errors
        raise credentials_exception
...
def register_user(user: UserCreate, admin_user: dict = Depends(admin_required)):
    ...
    user_data = {
        ...
        "user_type": user.privilege,  # Set user_type same as privilege
        "table": user.table,
...
def login_user(user_data: UserLogin):
    ...
    # Generate access token with both user_type and privilege
    access_token = create_access_token(
        data={
            "sub": user["username"],
            "user_type": user["privilege"],  # Use privilege as user_type
            "privilege": user["privilege"]   # Include privilege for consistency
        },
